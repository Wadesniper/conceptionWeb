...
if ($_SESSION['type'] != 'rb' || !isset($_SESSION['type'])){
    ...
    exit;
       ...
       if(!empty($rch_bat)){
           header('Location: gestion_bateau.php');
           }
            else{        
if(!empty($_POST)){
    extract($_POST);
    ...
    if(in_array($imageActuelExt, $ext_image_auto)){
        ...
        if(in_array($fileActuelExt, $ext_file_auto)){
            ...
            if(move_uploaded_file($file_tmp_name, $file_dest) && move_uploaded_file($image_tmp_name, $image_dest)){
                $DB->insert("INSERT INTO bateaux (bat_id_user, nom, pays, type, descp, pdf_url, img_url) VALUES (?, ?, ?, ?, ?, ?, ?)", array($bat_id_user, $nom, $pays, $type, $descp, $file_dest, $image_dest));
                echo 'Le bateau a bien été enregistré';
            }else {
                echo "la taille du pdf ou de la photo est trop grande";
            }
        }else {
    ...
    <body>
            ...
            <form method="POST" enctype="multipart/form-data">
                <div>Entrez le nom du bateau</div>
                    <input type="text" name="nom" placeholder="Le nom de votre bateau">
                ...
                <div>Entrez une description sommaire du bateau</div>
                    <textarea name="descp" rows=4 cols=40>Votre description de bateau</textarea><br/>
                <div>Uploadez votre pdf du bateau</div> 
                <input type="file" name="filePDF"/><br/>
                <div>Le pdf doit être inférieur à 2MO</div>
